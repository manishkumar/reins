 ...
 */
export function canDefer(payload?: Record<string, unknown>): boolean {
  let mode: string;
  try {
    mode = loadConfig((payload?.cwd as string) || undefined).holdTransport;
  } catch {
    mode = "auto";
  }
  if (mode === "deny") return false;
  if (mode === "defer") return true; // pinned by the user, evidence not consulted
  return isPrintMode();
}
...
export function isPrintMode(): boolean {
  ...
  if (process.platform === "win32") return false; // no ps; deny transport it is
  let argv: string;
  try {
    argv = execFileSync("ps", ["-o", "args=", "-p", pid], {
      encoding: "utf8",
      timeout: 2000,
 ...
 */
export function isPrintModeArgv(argv: string): boolean {
  const tokens = argv.trim().split(/\s+/);
  for (const tok of tokens.slice(1)) {
    if (!tok.startsWith("-")) return false; // reached the prompt/arguments
    if (PRINT_FLAGS.includes(tok) || tok.startsWith("--sdk-url")) return true;
  }
  return false;
}
