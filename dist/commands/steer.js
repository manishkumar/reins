...
async function cmdSteer(args) {
    ...
    if (rest[0] === "--clear" || rest[0] === "-c") {
        ...
        console.log(format_1.c.dim(sid ? `Cleared pending steering for ${short(sid)}.` : "Cleared pending global steering."));
        return 0;
    }
    ...
    if (replace || !hadPending) {
        (0, steering_1.writeSteering)(message, undefined, sid);
        console.log(format_1.c.green("✓ Steering queued") + target + ".");
    }
    else {
        const count = (0, steering_1.appendSteering)(message, undefined, sid);
        console.log(format_1.c.green(`✓ Added to pending steering${target} (${count} nudges queued).`));
...
function showPending(sid) {
    ...
    if (pending) {
        ...
        console.log(format_1.c.bold(`Pending steering${where} (delivers at the agent's next tool call):`));
        console.log("  " + pending.replace(/\n/g, "\n  "));
    }
    else {
        console.log(format_1.c.dim(sid ? `No steering queued for ${short(sid)}.` : "No steering queued."));
    }
    // Also surface any session-targeted nudges so they aren't forgotten.
    if (!sid) {
        const targeted = (0, steering_1.pendingTargetedSessions)();
        if (targeted.length) {
            console.log(format_1.c.dim("Targeted steering pending for: ") + targeted.map((t) => format_1.c.cyan(short(t))).join(", "));
        }
